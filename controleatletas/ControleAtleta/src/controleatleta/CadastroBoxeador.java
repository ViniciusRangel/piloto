...
public class CadastroBoxeador extends javax.swing.JFrame {
    ...
    private ControlePilotoCorrida controlePilotoCorrida;
    private PilotoCorrida piloto;
    ...
    private void habilitarDesabilitarCampos() {
        boolean registroSelecionado = (piloto != null);
    ...
    private void salvarRegistro() {
        ...
        if (novoRegistro == true) {
            piloto = new PilotoCorrida(jTextFieldNome.getText());
        } else {
            piloto.setNome(jTextFieldNome.getText());
        }
        piloto.setEndereco(endereco);
        piloto.setTelefones(telefones);
        piloto.setPremiacoes(premiacoes);
        piloto.setDataNascimento(dataNascimento);
        piloto.setAltura(Double.parseDouble(jTextFieldAltura.getText()));
        piloto.setNomeMae(jTextFieldNomeMae.getText());
        piloto.setNomePai(jTextFieldNomePai.getText());
        piloto.setPeso(Double.parseDouble(jTextFieldPeso.getText()));
        piloto.setCpf(jTextFieldCpf.getText());
        piloto.setRg(jTextFieldRg.getText());
        //piloto.setEnvergadura(Double.parseDouble(jTextFieldEnvergadura.getText()));
        //piloto.setTotalDerrotas(Integer.parseInt(jTextFieldTotalDerrotas.getText()));
        //piloto.setTotalDesistencias(Integer.parseInt(jTextFieldTotalDesistencias.getText()));
        //piloto.setTotalEmpates(Integer.parseInt(jTextFieldTotalEmpates.getText()));
        //piloto.setTotalLutas(Integer.parseInt(jTextFieldTotalLutas.getText()));
        //piloto.setTotalVitoriasNocaute(Integer.parseInt(jTextFieldTotalNocaute.getText()));
        piloto.setTotalVitorias(Integer.parseInt(jTextFieldTotalVitorias.getText()));

        switch (jComboBoxSexo.getSelectedIndex()) {
            case SEXO_MASCULINO_INDICE:
                piloto.setSexo(SEXO_MASCULINO_VALOR);
                break;
            case SEXO_FEMININO_INDICE:
                piloto.setSexo(SEXO_FEMININO_VALOR);
                break;
        }

        switch (jComboBoxCategoria.getSelectedIndex()) {
            case CATEGORIA_AMADOR_INDICE:
                piloto.setCategoria(CATEGORIA_AMADOR_VALOR);
                break;
            case CATEGORIA_PROFISSIONAL_INDICE:
                piloto.setCategoria(CATEGORIA_PROFISSIONAL_VALOR);
                break;
        }
        ...
        if (novoRegistro == true) {
            controlePilotoCorrida.adicionar(piloto);
        }
        modoAlteracao = false;
        novoRegistro = false;
    ...
    private void carregarListaBoxeadores() {
        ArrayList<PilotoCorrida> listaPilotosCorridas = controlePilotoCorrida.getlistaPilotoCorridas();
        DefaultTableModel model = (DefaultTableModel) jTableListaBoxeadores.getModel();
        model.setRowCount(0);
        for (PilotoCorrida p : listaPilotosCorridas) {
            model.addRow(new String[]{p.getNome(), p.getCpf()});
        }
...
private void jButtonPesquisarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPesquisarActionPerformed
    ...
    private void pesquisarBoxeador(String nome) {
        ...
        if (boxeadorPesquisado == null) {
            ...
            this.piloto = boxeadorPesquisado;
            this.preencherCampos();
            this.habilitarDesabilitarCampos();
        }
    }
